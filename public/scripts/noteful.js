    ...
    $('.js-note-edit-form').on('submit', function (event) {
      ...
      if (noteObj.id) {
        ...
        .then(updateResponse => {
          ...
          return (store.currentSearchTerm)
        })
        .then( (searchTerm) => {
        ...
        .then(createResponse => {
          ...
          return (api.search(store.currentSearchTerm, searchResponse));
        })
        .then(searchResult => {
            store.notes = searchResult;
            render();
          });
  ...
  function handleNoteDeleteClick() {
    $('.js-notes-list').on('click', '.js-note-delete-button', event => {
      ...
      api.remove(noteId).then(() => {
        resolve( api.search(store.currentSearchTerm, searchResponse)
        )}).then(searchResponse => {
          store.notes = searchResponse;
